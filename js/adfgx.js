...
{ 
    if(keysquare.length != 25){ alert("keysquare must be 25 characters in length containing letters a-z (no j)"); return; }
...
function decrypt(ciphertext, keysquare, keyword)
{
    ...
    if(re.test(ciphertext)){alert("ciphertext can only contain A,D,F,G or X characters."); return;};
